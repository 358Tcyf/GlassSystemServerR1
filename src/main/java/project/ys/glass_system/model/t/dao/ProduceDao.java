...
public interface ProduceDao extends JpaRepository<ProduceItem, Integer> {
    ...
    @Query("SELECT COALESCE(SUM(produceItem.water),0) FROM gls_produce_table produceItem WHERE produceItem.time BETWEEN ?1 and ?2")
    float sumWaterByTimeBetween(LocalDateTime time, LocalDateTime time2);

    @Query("SELECT COALESCE(SUM(produceItem.electricity),0) FROM gls_produce_table produceItem WHERE  produceItem.time BETWEEN ?1 and ?2")
    float sumElectricityByTimeBetween(LocalDateTime time, LocalDateTime time2);

    @Query("SELECT COALESCE(SUM(produceItem.material),0) FROM gls_produce_table produceItem WHERE   produceItem.time BETWEEN ?1 and ?2")
    float sumMaterialByTimeBetween(LocalDateTime time, LocalDateTime time2);

    @Query("SELECT COALESCE(SUM(produceItem.coal),0) FROM gls_produce_table produceItem WHERE  produceItem.time BETWEEN ?1 and ?2")
    float sumCoalByTimeBetween(LocalDateTime time, LocalDateTime time2);
